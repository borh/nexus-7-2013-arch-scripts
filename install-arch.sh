...
busybox wget http://busybox.net/downloads/binaries/latest/busybox-armv7l -O "/data/local/tmp/arch/arch-busybox"
...
arch-busybox wget -c http://tw.mirror.archlinuxarm.org/os/ArchLinuxARM-trimslice-latest.tar.gz

echo "Making 10GB image..."
arch-busybox dd if=/dev/zero of=${CHROOT}.img bs=1M seek=10000 count=1

echo "Formating image..."
arch-busybox mke2fs -L arch-image -F ${CHROOT}.img
arch-busybox mkdir ${CHROOT}

echo "Making devices..."
arch-busybox mknod /dev/loop256 b 7 256
...
arch-busybox gunzip ArchLinuxARM-trimslice-latest.tar.gz -c | arch-busybox tar x -f - -C ${CHROOT}
